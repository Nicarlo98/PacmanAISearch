    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                cost = 1
                successors.append(
                    ((nextState, tuple(cornersVisited)), action, cost, ))
...
def cornersHeuristic(state, problem):
    ...
    # corners pacman has not been too
    unvisitedCorners = []
    ...
    if len(unvisitedCorners) > 0:
        # get the farthest unvisited corner from the current location
        farthestCorner = None
        farthestDistance = -1
        for corner in unvisitedCorners:
            distance = mazeDistance(
                currentLocation, corner, problem.startingGameState)
            if distance > farthestDistance:
                farthestCorner = corner
                farthestDistance = distance

        # get the closest unvisited corner from the current location
        closestCorner = None
        ...
        # add the distance from the current location to the closest unvisited corner
        heuristic += closestDistance

        # add the distance from the closest unvisited corner to the farthest unvisited corner
        heuristic += mazeDistance(closestCorner,
                                  farthestCorner, problem.startingGameState)

    return heuristic


def findClosestPoint(location, goals):
    """
    A helper function for identifying the closest goal location to a given location.
    """
    min_dist, closest_goal = float('inf'), None
    for i, goal in enumerate(goals):
        dist = util.manhattanDistance(location, goal)
        if dist < min_dist:
            min_dist, closest_goal = dist, i
    return closest_goal, min_dist


def findFarthestPoint(location, goalArray):
    """
    A helper function for corner detection
    """

    furthestPoint = 0
    furthestPointCost = util.manhattanDistance(location, goalArray[0])

    for i in range(len(goalArray)):
        # Calculate the Manhattan distance between the current location and the goal corner
        cornerLocation = goalArray[i]
        lengthToCorner = util.manhattanDistance(location, cornerLocation)

        if lengthToCorner > furthestPointCost:
            furthestPoint = i
            furthestPointCost = lengthToCorner

    return (furthestPoint, furthestPointCost)
...
def foodHeuristic(state, problem):
    ...
    # Find the closest food dots to Pacman
    closestFood = min(
        foodList, key=lambda x: util.manhattanDistance(position, x))

    # Calculate the Manhattan distance from Pacman to the closest food dots
    heuristic = util.manhattanDistance(position, closestFood)
