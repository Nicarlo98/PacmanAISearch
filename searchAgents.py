...
class CapsuleSearchProblem:
    ...
    def __init__(self, startingGameState):
        ...
        self.start = (self.init_pos, self.foodGrid, self.capsulesGrid)
        self.startingGameState = startingGameState
    ...
    def getSuccessors(self, state):
        ...
        for direction in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[next_x][next_y]:
                ...
                #print(str((state[2])[x][y]))
                if ((state[2])[next_x][next_y]) == True:
                    cost = 0

                #change the succ coord capsule to false
                nextCapsule[next_x][next_y] = False
                
                successors.append( ( ((next_x, next_y), nextFood, nextCapsule), direction, cost) )
        return successors
...
def capsuleProblemHeuristic(state, problem):
    ...
    position, foodGrid, capsuleGrid = state
    foodList = foodGrid.asList()

    tree = [position]
    totalDist = 0
    while len(foodList) > 0:
        unconnect = util.PriorityQueue()
        for food in foodList:
            for v in tree:
                dist = manhattanCapsuleDistance(food, v, capsuleGrid)
                unconnect.push((dist, food, v),dist)
        smallest = unconnect.pop()
        totalDist = totalDist + smallest[0]
        foodList.remove(smallest[1])
        tree.append(smallest[1])


    return totalDist

def manhattanCapsuleDistance(xy1, xy2, capsuleGrid):
    capsules = 0
    minX = min(xy1[0], xy2[0])
    maxX = max(xy1[0], xy2[0])
    minY = min(xy1[1], xy2[1])
    maxY = max(xy1[1], xy2[1])

    mDist = (maxX - minX) + ( maxY - minY)

    for i in range(minX, (maxX + 1)):
        for j in range(minY, (maxY + 1)):
            if (capsuleGrid[i][j]) == True:
                capsules = capsules + 1

    if capsules > (mDist - 1):
        capsules = mDist - 1
           
    return mDist - capsules
